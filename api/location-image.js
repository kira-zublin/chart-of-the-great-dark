...
export async function GET(req) {
  return guarded(async () => {
    ...
    const id = idOf(req); const location = (await sql`SELECT access_level FROM locations WHERE id = ${id}`)[0];
    if (!location || (location.access_level !== 'accessible' && profile.role !== 'gm')) return error('Location unavailable', 404);
    const image = (await sql`SELECT mime_type, bytes FROM location_images WHERE location_id = ${id}`)[0];
...
export async function PUT(req) {
  return guarded(async () => {
    ...
    const id = idOf(req), location = (await sql`SELECT id, kind FROM locations WHERE id = ${id}`)[0];
