...
test('local Ship City slice links districts and preserves repeatable seed data', async () => {
  ...
  try {
    ...
    assert.ok(cityLinks.some(link => link.to_id === 'inner-sanctum'));
    ...
    for (const [hub, expected] of [
      ['cave-gardens', ['purple-meadows', 'hydroponic-terraces', 'seed-archive']],
      ['turbine-halls', ['mosaicists-walk', 'service-gallery', 'cherolab-gate']],
      ['hull-town', ['wreck-alley', 'rouzads-emporium', 'chain-walks']],
      ['inner-sanctum', ['ziggurat-nine-icons', 'palace-daars', 'marakanda-tower']]
    ]) {
    ...
    for (const id of ['seed-archive', 'cherolab-gate', 'palace-daars', 'marakanda-tower']) {
    ...
    const crossing = (await sql.query("SELECT kind, grid FROM locations WHERE id = 'chain-walks'"))[0];
    assert.equal(crossing.kind, 'delve');
    const crossingGrid = JSON.parse(crossing.grid);
    assert.deepEqual(crossingGrid.entry, [1, 4]);
    assert.equal(crossingGrid.rooms.length, 3);
    assert.ok(crossingGrid.blocked.some(([x, y]) => x === 4 && y === 4));
    assert.ok(!crossingGrid.blocked.some(([x, y]) => x === 5 && y === 4));
