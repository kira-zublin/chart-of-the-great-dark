...
const places = [
  ...
  ['cave-gardens', 'poi', 'ship-city', 'Cave Gardens', 'Ship City’s green lung fills connected caverns with orchards, hydroponic ponds and tree-lined paths. The Gardeners Guild tends both its abundant life and its carefully guarded secrets.', 'Follow the scent of herbs into the asteroid’s living heart.', '', '', 'accessible'],
  ['turbine-halls', 'poi', 'ship-city', 'Turbine Halls', 'Reactors, tanks and machinery sustain the city beneath high vaulted stone. Machinists work among geometric murals and bright painted dwellings while Gray Caps guard the Cherolab.', 'The engine of Ship City hums behind painted industrial walls.', '', '', 'accessible'],
...
export async function seedShipCitySlice(sql = db()) {
  ...
  for (const [id, kind, parent, title, description, teaser, quote, speaker, access] of places) {
    await sql.query('INSERT OR IGNORE INTO locations (id, kind, parent_id, title, description, teaser, quote, quote_speaker, access_level, grid) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [id, kind, parent, title, description, teaser, quote, speaker, access, id === 'warehouse-murk' ? murkGrid : '{}']);
  }
