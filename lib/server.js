...
export async function currentProfile(req, sql = db()) {
  ...
  const rows = await sql`SELECT p.id, p.name, p.role FROM sessions s JOIN profiles p ON p.id = s.profile_id WHERE s.token_hash = ${digest(token)} AND s.expires_at > now() LIMIT 1`;
  return rows[0] || null;
}
export async function createSession(req, profile, sql = db()) {
  const token = randomBytes(32).toString('hex');
  await sql`INSERT INTO sessions (token_hash, profile_id, expires_at) VALUES (${digest(token)}, ${profile.id}, now() + interval '30 days')`;
