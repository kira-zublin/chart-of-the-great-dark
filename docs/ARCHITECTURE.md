...
Chat renders each roll as a dice card (`chat-ui.js`, styled in `ui-theme.css`). The card is presentation only: it draws the stored dice and interpretation, carries the export text for screen readers, and animates only rolls that arrive after chat has loaded.
