...
`db/004_chat.sql` adds one shared campaign message stream. Apply it with `scripts/migrate-004.js` to each intended database before deploying chat code there. Messages retain player and character name snapshots, server timestamps, text or complete roll results, and an optional character reference. Deleting a character clears the reference but preserves the name and log; its portrait then falls back to the generic asset. Every chat API request requires a session. Sending with a character requires current ownership of a PC, except that a GM may use any character. All signed-in users can read history and download a UTF-8 text log for inclusive UTC dates.

The server performs all d6 rolls with cryptographic randomness. Action rolls use the stored attribute and one stored talent level, plus a player-entered base-dice modifier and optional gear dice. The base pool has a minimum of one die; every six on base or gear dice is a success. The first version records rolls without automatically applying Hope loss, gear wear, or sheet changes. Chat updates poll the API every two seconds and on tab return, using message IDs to handle ordering and reconnects. The initial read returns the latest 100 messages; downloads can include the full requested date range.
