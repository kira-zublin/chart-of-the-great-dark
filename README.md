...
Vercel Preview builds apply the additive chat migration automatically to their newly provisioned Turso branch. For Production, back up its database and apply `node --env-file=.env.production-migration.local scripts/migrate-004.js` before merging chat. The migration is safe to rerun. Signed-in players share one chat stream with plain text, clickable links, d6 rolls, and action rolls. Chat history can be downloaded as a text file over an inclusive UTC date range.
