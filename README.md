...
The local preview seeds four Ship City districts as Hubs: Aluminum Bay, the Chasm, Cave Gardens, and Turbine Halls. Their selected inner locations include Vistas, restricted Guild thresholds, and two Explorables. The remaining smaller districts stay as Points of Interest on the main city map. Cave Gardens and Turbine Halls expand the first slice with original playable interpretations of the Gardeners' growing rooms and the Machinists' work passages; the Seed Archive, Hydroponic Terraces, Mosaicists' Walk, and Service Gallery are prototype locales rather than named book sites. Every Hub, Vista, and Explorable in these four districts has generated watercolor art. The seed removes only the original sample markers from Ship City's map and is limited to the in-memory local preview. `scripts/seed-ship-city-slice.js` must not be run against a shared campaign database without reviewing existing locations and character positions first.
