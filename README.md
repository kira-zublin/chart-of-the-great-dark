...
The local preview now seeds a first Ship City slice: Aluminum Bay, the Chasm, selected inner locations, and smaller district Points of Interest. It removes only the original sample markers from Ship City's map. Both district Hubs, Warehouse Murk, every enterable Vista in this slice, and the restricted Astrolaab Tower have generated map or scene art. Points of Interest use a crop of their parent map. The seed is deliberately limited to the in-memory local preview. `scripts/seed-ship-city-slice.js` must not be run against a shared campaign database without reviewing existing locations and character positions first.
